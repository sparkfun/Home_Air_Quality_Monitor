...
// Licence Creative Commons Attribution-ShareAlike 4.0 International (CC
// BY-SA 4.0)
//
...
void Screen_EPD_EXT3_Fast::COG_initial(uint8_t updateMode) {
  ...
  if ((u_codeExtra & FEATURE_FAST) and
      (updateMode != UPDATE_GLOBAL)) // Specific settings for fast update
  {
  ...
  if ((u_codeExtra & FEATURE_FAST) and (updateMode != UPDATE_GLOBAL) and
      _flag50) {
...
void Screen_EPD_EXT3_Fast::COG_sendImageDataFast() {
  ...
  memcpy(previousBuffer, nextBuffer,
         u_frameSize); // Copy displayed next to previous
}
...
// === Class section
//
Screen_EPD_EXT3_Fast::Screen_EPD_EXT3_Fast(eScreen_EPD_EXT3_t eScreen_EPD_EXT3,
                                           pins_t board) {
...
void Screen_EPD_EXT3_Fast::begin() {
  ...
  u_bufferSizeH =
      _screenSizeH / 8; // horizontal = small size 112 / 8, 1 bit per pixel
  ...
  // 9.69 and 11.98 combine two half-screens, hence two frames with adjusted
  // size
  switch (u_codeSize) {
  case 0x96: // 9.69"
  case 0xB9: // 11.98"
  ...
  // Report
  Serial.println(formatString("= Screen %s %ix%i", WhoAmI().c_str(),
                              screenSizeX(), screenSizeY()));
  Serial.println(formatString("= PDLS %s v%i.%i.%i", SCREEN_EPD_EXT3_VARIANT,
                              SCREEN_EPD_EXT3_RELEASE / 100,
                              (SCREEN_EPD_EXT3_RELEASE / 10) % 10,
                              SCREEN_EPD_EXT3_RELEASE % 10));
...
String Screen_EPD_EXT3_Fast::WhoAmI() {
  ...
  return formatString("iTC %i.%02i\"%s", _screenDiagonal / 100,
                      _screenDiagonal % 100, work);
}
...
void Screen_EPD_EXT3_Fast::_setPoint(uint16_t x1, uint16_t y1,
                                     uint16_t colour) {
