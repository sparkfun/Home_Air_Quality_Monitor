...
class SpiffsBuildConfig(object):
    def __init__(self,
        ...
        self.OBJ_DATA_PAGE_HEADER_LEN = self.obj_id_len + \
            self.span_ix_len + SPIFFS_PH_FLAG_LEN

        pad = 4 - (4 if self.OBJ_DATA_PAGE_HEADER_LEN %
                   4 == 0 else self.OBJ_DATA_PAGE_HEADER_LEN % 4)
        ...
        if aligned_obj_ix_tables:
            self.OBJ_INDEX_PAGES_HEADER_LEN_ALIGNED = (
                self.OBJ_INDEX_PAGES_HEADER_LEN + SPIFFS_PAGE_IX_LEN - 1) & ~(SPIFFS_PAGE_IX_LEN - 1)
            self.OBJ_INDEX_PAGES_HEADER_LEN_ALIGNED_PAD = self.OBJ_INDEX_PAGES_HEADER_LEN_ALIGNED - \
        ...
        if remaining >= 2:
            for i in range(remaining):
                if i == remaining - 2:
                    ...
                    self.obj_ids.append(
                        (empty_obj_id_dict[self.build_config.obj_id_len], SpiffsObjDataPage))
                self.obj_ids_limit -= 1
...
class SpiffsBlock(object):
    def _reset(self):  # type: () -> None
        ...
        self.cur_obj_id = 0
        # type: typing.Optional[SpiffsObjIndexPage]
    ...
    def _register_page(self, page):  # type: (TSP) -> None
        if isinstance(page, SpiffsObjDataPage):
            ...
            self.cur_obj_idx_page.register_page(
                page)  # can raise SpiffsFullError
...
class SpiffsFS(object):
    ...
    def to_binary(self):  # type: () -> bytes
        ...
        if self.build_config.use_magic:
            ...
            all_blocks.append(
                b'\xFF' * (self.img_size - len(all_blocks) * self.build_config.block_size))
...
def main():  # type: () -> None
    ...
    if not os.path.exists(args.base_dir):
        raise RuntimeError(
            'given base directory %s does not exist' % args.base_dir)
    ...
    with open(args.output_file, 'wb') as image_file:
        ...
        for root, dirs, files in os.walk(args.base_dir, followlinks=args.follow_symlinks):
            for f in files:
                ...
                spiffs.create_file(
                    '/' + os.path.relpath(full_path, args.base_dir).replace('\\', '/'), full_path)
